...
if (window.trustedTypes && window.trustedTypes.createPolicy) {
    if (!window.trustedTypes.defaultPolicy) {
        try {
            ...
            const cleanTrustedTypeString = (string) => String(string || '')
                .replace(/\s+on\w+\s*=\s*['"][^'"]*['"]/gi, '')
                .replace(/javascript\s*:|vbscript\s*:|data\s*:\s*text\/html/gi, '');
            const cleanTrustedTypeUrl = (string) => {
                const value = String(string || '').trim();
                const compact = value.replace(/[\u0000-\u001F\u007F\s]+/g, '').toLowerCase();
                if (/^(javascript:|vbscript:|data:text\/html)/.test(compact)) return '';
                return value;
            };
            window.trustedTypes.createPolicy('default', {
                createHTML: (string) => {
                    return cleanTrustedTypeString(string);
                },
                createScript: () => '',
                createScriptURL: (string) => cleanTrustedTypeUrl(string)
            });
        } catch (e) {
...
class AudioEngine {
    ...
    speak(text) {
        ...
        // Keep the platform fully static and privacy-friendly: never stream external TTS audio.
